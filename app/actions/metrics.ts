...
export async function getMonthMetrics(monthsBack: number = 12) {
  ...
  const now = new Date();
  const habitsList = await prisma.habit.findMany({ where: { archived: false }, select: { id: true, targetPerWeek: true } });
  const habitIds = habitsList.map((h) => h.id);
  const habitTargets = Object.fromEntries(habitsList.map((h) => [h.id, h.targetPerWeek]));
  ...
  for (let i = 0; i < monthsBack; i++) {
    ...
    const monthStartUTC = localDayStartUTC(format(monthStart, "yyyy-MM-dd"));
